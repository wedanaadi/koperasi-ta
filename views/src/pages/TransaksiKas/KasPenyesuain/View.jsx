...
export default function View() {
  ...
  const fetchDatas = async () => {
    let url = `${baseUrl}/kas?page=${currentPage}&perpage=${pagination.value}&jenis=penyesuain`;
    if (onSearch) {
      setCurrentPage(1);
      url += `&search=${onSearch}`;
    }
  ...
  const deleteKasPenyesuainMutation = useMutation({
    ...
    onSuccess: () => {
      setCurrentPage(1);
      queryClient.invalidateQueries({ queryKey: ["penyesuain"] });
      toastChange({
        id: "NotifKP",
        content: {
          title: "Delete Data",
          description: "Delete data Penyesuain Kas Successfuly",
  ...
  const handleHapus = (id) => {
    if (confirmDelete) {
      deleteKasPenyesuainMutation.mutate({
        id: id,
        token: tokenLogin,
        tipe: "penyesuain",
      });
  ...
  const handleCetak = (id) => {
    window.open(`${import.meta.env.VITE_BACKEND_PUBLIC}/kas/bukti?id=${id}&lokasi=${dataSetting?.lokasi ? dataSetting.lokasi : 'Bali'}&title=sesuai&alamat=${dataSetting?.alamat ? btoa(dataSetting.alamat) : '-'}`, '_blank');
  }
    ...
    <>
      ...
      <div className="bg-white card">
        <div className="border-second card-header">
          <h3 className="mb-0 text-lg font-bold">Data Penyesuain Kas</h3>
        ...
        <div className="card-body">
              ...
              <div className="min-w-full py-2 sm:px-6 lg:px-8 xl:px-0">
                <table className="min-w-full border-2 border-third text-center text-base font-light">
                  ...
                  <tbody>
                    ...
                    {penyesuains?.data.length > 0
                      ? penyesuains?.data.map((data, index) => (
                          <tr
          ...
          {penyesuains ? (
            <Pagination
              ...
              totalCount={penyesuains.total}
              pageSize={penyesuains.per_page}
