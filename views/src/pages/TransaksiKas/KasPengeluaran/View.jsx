...
export default function View() {
  ...
  const fetchDatas = async () => {
    let url = `${baseUrl}/kas?page=${currentPage}&perpage=${pagination.value}&jenis=pengeluaran`;
    if (onSearch) {
      setCurrentPage(1);
      url += `&search=${onSearch}`;
    }
  ...
  const {
    ...
    networkMode: `always`,
    queryKey: ["kaskeluar", currentPage, pagination],
    queryFn: fetchDatas,
  });
  ...
  const deleteKasMasukMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toastChange({
        ...
        content: {
          title: "Delete Data",
          description: "Delete data Pengeluaran Kas Successfuly",
  ...
  const handleCetak = (id) => {
    window.open(`${import.meta.env.VITE_BACKEND_PUBLIC}/kas/bukti?id=${id}&lokasi=${dataSetting?.lokasi ? dataSetting.lokasi : 'Bali'}&title=keluar&alamat=${dataSetting?.alamat ? btoa(dataSetting.alamat) : '-'}`, '_blank');
  }
    ...
    <>
      ...
      <div className="bg-white card">
        <div className="border-second card-header">
          <h3 className="mb-0 text-lg font-bold">Data Pengeluaran Kas</h3>
        ...
        <div className="card-body">
                ...
                <table className="min-w-full border-2 border-third text-center text-base font-light">
                  ...
                  <tbody>
                    ...
                    {pengeluarans?.data.length > 0
                      ? pengeluarans?.data.map((data, index) => (
                          <tr
          ...
          {pengeluarans ? (
            <Pagination
              ...
              totalCount={pengeluarans.total}
              pageSize={pengeluarans.per_page}
