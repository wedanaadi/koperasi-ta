...
export default function Index() {
  ...
  const getData = () => {
    ...
    }/biayaadmin?page=${currentPage}&perpage=${"10"}`;
    if (onSearch) {
      setCurrentPage(1);
      url += `&search=${onSearch}`;
    }
  ...
  const handleHapus = (id) => {
    ...
    if (confirm(text) == true) {
      axios
      .delete(`${import.meta.env.VITE_BACKEND_API}/biayaadmin/${id}`,{})
      .then((res) => {
        getData()
        toastChange({
          id: "NotifdeleteBiayaAdmin",
          content: {
            title: "Delete Data",
            description: "Delete Biaya Admin Successfuly",
  ...
  return (
    <div className="bg-white card">
      <div className="border-second card-header">
        <h3 className="mb-0 text-2xl font-semibold">Biaya Administrasi</h3>
                    ...
                    {dataApi?.data?.data.length > 0 ? (
                      dataApi?.data?.data.map((data, index) => (
                        <tr className="border-b font-medium" key={index}>
                          ...
                          <td className="whitespace-nowrap border-r px-6 py-4 text-left">
                            {data.is_anggota == '1' ? 'Untuk Anggota' : 'Untuk Bukan Anggota'}
                          </td>
